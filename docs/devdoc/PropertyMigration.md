...
- `AppDbApi.rt2Orm`, `ToORM`, `DB.Insert`, `DB.Select` 등에서 속성 복제가 `PropertiesJson` 기준으로만 이루어지도록 수정한다.
...
## 7. 직렬화/복제 유틸리티
- `DsCopy.Properties.fs`, `DsCompare.Objects.fs` 등 속성을 비교/복제하는 모듈에서 `Properties` 중심으로 동작하도록 변경한다.
...
- Json 비교 테스트는 `Properties` 내부 필드까지 검증하도록 업데이트한다.
