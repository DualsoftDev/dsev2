...
| 3    | ReadTag ⊕ WriteTag      | 호출자와 응답자가 맥락에 따라 읽기쓰기 해석됨 |
...
## 🌐 DS Duality 이중성: Case 3 — ApiCall → Tag → ApiDef
...
DS 시스템에서 동일한 System이라도 생성 방식과 활용 방식에 따라 **Instance** 또는 **Reference**라는 두 가지 역할을 수행합니다. 이 이중성은 실행 가능성과 참조 목적의 차이를 반영하며, 시스템 정의와 운용에 유연함을 제공합니다.

* **Instance**: 프로젝트에서 \*\*`new`로 직접 생성한 시스템\`\*\*으로, 시스템의 원본 정의이며 내부 구조와 설정을 모두 포함합니다. Instance는 \*\*능동(Active)\*\*일 경우에만 직접 실행이 가능하며, \*\*수동(Passive)\*\*일 경우에는 외부의 요청에만 응답합니다.

* **Reference**: 기존에 다른 프로젝트에서 정의된 Instance를 참조하는 객체입니다. **직접 생성된 구조가 아니므로 항상 Passive**로 동작하며, 참조 기반으로만 활용됩니다.

> ⊕: 하나의 시스템은 최초 생성 프로젝트에서는 Instance이고, 그 이후의 다른 프로젝트에서는 동일 시스템을 Reference로 참조할 수 있습니다.

이 구조는 객체지향 개념의 "객체 생성(new)"과 "참조(reference)"의 차이와 동일합니다.

### 2. 이중성 구조 해석

| 항목       | Instance             | Reference            |
| -------- | -------------------- | -------------------- |
| 정의 방식    | new로 직접 생성           | 기존 시스템 참조            |
| 실행 가능성   | ✅ Active일 경우에만 실행 가능 | ❌ 실행 불가 (항상 Passive) |
| 내부 로직 소유 | ✅ 있음                 | ❌ 없음                 |
| 시스템 위치   | 최초 생성 프로젝트           | 이후 참조 프로젝트           |
| 목적       | 시스템 원형 정의 및 실행       | 기존 시스템 재사용           |
...
* 프로젝트 A는 SystemX를 직접 생성한 원본이므로 Instance입니다.
* 프로젝트 B는 SystemX를 참조만 하므로 Reference입니다.

### 4. 객체 이중성의 특징

* **Instance는 시스템 정의의 중심**으로, 모든 내부 상태, 설정, 구조를 포함합니다.
* **Reference는 정의된 Instance를 간접 활용**하기 위한 참조로, 실행 책임을 갖지 않으며 재사용 중심입니다.
* 이중성은 객체의 정의 방식과 관점에 따라 구분됩니다.

### 5. 설계 효과 및 운영 전략

new로 생성된 시스템은 Instance, 참조하는 시스템은 Reference로 명확하게 구분되며, Active/Passive에 따른 실행 가능성까지 설명됩니다.

### 6. 설계 및 운영 주의사항

* **처음 생성 프로젝트에서는 반드시 Instance로 시스템 구성**되어야 합니다.
* **Reference는 Instance의 설정 및 구조를 변경할 수 없습니다.**
* **순환 참조를 피해야 하며**, 시스템 정의는 명확한 계층 구조를 가져야 합니다.

### 7. 결론

**Instance ⊕ Reference 이중성**은 DS 시스템의 **시스템 정의 및 재사용을 위한 필수 구조**입니다.

* **Instance**: 생성 프로젝트에서 정의된 시스템으로, 실행 주체가 될 수 있습니다.
* **Reference**: 다른 프로젝트에서 기존 시스템을 참조하여 사용하는 객체입니다.
* 실행 가능 여부는 Active/Passive 설정에 따라 결정되며, Reference는 항상 Passive입니다.
