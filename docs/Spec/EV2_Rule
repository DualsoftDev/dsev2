...
DS(Dualsoft) 시스템은 복잡한 산업 자동화 환경에서 유연하고 확장 가능한 시스템 설계를 위해 **이중성(Duality)** 개념을 핵심으로 합니다. 이중성은 하나의 구성 요소가 상황과 맥락에 따라 서로 다른 역할을 수행할 수 있음을 의미하며, 이를 통해 시스템의 재사용성, 모듈성, 유연성을 극대화합니다.

DS의 이중성은 \*\*구조적 이중성(Structural Duality)\*\*과 \*\*실행적 이중성(Execution Duality)\*\*으로 분류됩니다:

* **구조적 이중성 (Cases 1-4)**: 시스템의 설계, 정의, 연결 관계에서 나타나는 이중적 역할
* **실행적 이중성 (Cases 5-8)**: 런타임에서 상태, 신호, 데이터 처리 과정에서 나타나는 이중적 특성

### 핵심 원리

1. **동적 역할 전환**: 동일한 구성 요소가 상황에 따라 능동적 또는 수동적 역할을 수행
2. **상호 보완적 관계**: 이중성의 각 측면은 서로를 보완하며 완전한 기능을 구성
3. **맥락 의존적 해석**: 동일한 요소가 사용되는 맥락에 따라 다른 의미와 동작을 가짐
4. **계층적 추상화**: 외부에서는 단순하게, 내부에서는 복잡하게 구성되는 다층 구조

---

## 구조적 이중성 요약 (Cases 1-4)

| Case | 구성 요소                            | 이중성 설명                       |
| ---- | -------------------------------- | ---------------------------- |
| 1    | System ⊕ Device                  | 호출 방향에 따라 능동적/수동적 역할 수행      |
| 2    | Work ⊕ Call                      | 외부에선 단일 실행 단위, 내부에선 복수 호출 흐름 |
| 3    | ApiCall ⊕ ApiDef                 | 호출자/응답자의 역할 전환 가능성           |
| 4    | Instance ⊕ Reference             | 설계, 실행, 참조로 분화되는 역할 이중성      |

---

## 실행적 이중성 요약 (Cases 5-8)

| Case | 구성 요소                    | 이중성 설명               |
| ---- | ------------------------ | -------------------- |
| 5    | WorkBit = R ⊕ G ⊕ F ⊕ H  | FSM 기반 실행 상태 전이 표현   |
| 6    | φ(θ) = 위상 표현 ⊕ 상태 유추     | 센서 조합 기반 실행 위치 추정    |
| 7    | Tag = Semantic ⊕ Binding | 의미 정보와 물리 주소의 이중 맵핑  |
| 8    | Arrow = Start ⊕ Reset    | 트리거 조건과 초기화 조건 분리 제어 |
...
* 재사용 가능한 구성 요소 정의 가능
* 구조 설계와 실행 제어 간의 경계를 명확하게 설정

#### 실행 측면

* 상태 전이 명확화로 안정성 향상
* 트리거/리셋 제어로 예측 가능한 동작 구현

#### 시스템 통합 측면

* 모듈 간 의존도 감소 및 상호 운용성 향상
* 다중 프로젝트 간의 참조/인스턴싱 유연성 확보

---

각 항목의 상세한 설명은 Case 1부터 8까지 개별 문서를 통해 확인할 수 있습니다.
...
## DS Duality 이중성: Case 2 — Work ⊕ Call

### 1. 개념 소개

DS에서 Work는 단순한 실행 블록이 아니라 **여러 Call을 포함하는 컨테이너이며 동시에 하나의 실행 단위**로 작동합니다. 즉, **Work ⊕ Call 이중성**은 내부 복잡성과 외부 단순성을 동시에 가지는 구조입니다.

* **Work**: 외부에서는 단일 실행 단위처럼 보임
* **Call**: 내부적으로 구성된 실행 요소들의 모음

> ⊕: 하나의 Work는 내부적으로 Call 다수 포함, 외부에서는 단일 블록처럼 보임

### 2. 구조 정의

#### 2.1 Work

* **정의**: 내부에 여러 Call을 포함하며, 외부에서는 하나의 추상 노드로 동작
* **특징**:

  * 내부 실행 순서는 DAG(비순환 방향 그래프)로 표현
  * 하나의 입력/출력 인터페이스를 가짐

#### 2.2 Call

* **정의**: 실제 외부 API(System)의 ApiDef를 호출하는 실행 블록
* **특징**:

  * 각 Call은 하나의 작업 단위를 수행
  * Work 내부에서 종속성과 순서를 가짐
...
Work A
  ├─ Call 1 → B.ApiDef
  ├─ Call 2 → C.ApiDef (Call 1 종료 후 실행)
  └─ Call 3 → D.ApiDef (병렬 실행)
```

* 외부에서는 "Work A" 단일 블록처럼 보임
* 내부에서는 Call 1\~3이 흐름 제어됨

### 4. 이중성의 장점

* **모듈화**: Call 구성만 바꿔서 Work 재활용 가능
* **단순화**: 외부에는 Work 단위만 노출
* **관찰성 향상**: Call 단위로 로그 기록 및 상태 추적 가능

### 5. 주의사항

#### 5.1 DAG 구조 유지

* Call 간 흐름은 순환(Cycle)이 없어야 함
* Work 내 논리 루프는 설계자 의도에 따라 경고 처리

#### 5.2 외부 흐름 연계

* Work 간 연결은 Arrow로 구성되며 Start/Reset 조건이 필요

#### 5.3 상태 추적 경계

* 외부에서는 Work 상태만 확인 가능
* 내부 Call의 상태는 별도 추적 구조 필요

### 6. 매트릭스 요약

| 항목   | 역할        | 구조           | 실행 단위 | 관측 단위          |
| ---- | --------- | ------------ | ----- | -------------- |
| Work | Container | DAG(Call 포함) | ✅     | ✅              |
| Call | API 호출    | 노드           | ✅     | ❌ (Work 내부에서만) |

### 7. 정리

Work는 단일 실행 블록처럼 보이지만, 내부적으로는 복잡한 Call 흐름을 포함합니다. 이 이중성을 통해 복잡한 실행 논리를 깔끔하게 감쌀 수 있으며, 유지보수성과 재사용성이 크게 향상됩니다.
