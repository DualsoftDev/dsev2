...
let ``UserFBRegistry - FC 등록 및 조회`` () =
    ...
    builder.AddInput("x", DsDataType.TInt)
    builder.AddOutput("y", DsDataType.TInt)
...
let ``UserFBRegistry - GetAllFCs 모든 FC 조회`` () =
    ...
    fc1.AddInput("x", DsDataType.TInt)
    fc1.AddOutput("y", DsDataType.TInt)
    fc1.SetBody(intExpr 1)

    let fc2 = FCBuilder("FC2")
    fc2.AddInput("a", DsDataType.TBool)
    fc2.AddOutput("b", DsDataType.TBool)
...
let ``UserFBRegistry - GetAllFBs 모든 FB 조회`` () =
    ...
    fb1.AddInput("input", DsDataType.TBool)
    fb1.AddOutput("output", DsDataType.TBool)
    fb1.AddStatement(Assign(0, DsTag.Bool("output"), boolExpr true))

    let fb2 = FBBuilder("FB2")
    fb2.AddInput("enable", DsDataType.TBool)
    fb2.AddOutput("done", DsDataType.TBool)
...
let ``UserFBRegistry - ValidateAll 모든 FC/FB 검증`` () =
    ...
    fc.AddInput("x", DsDataType.TInt)
    fc.AddOutput("y", DsDataType.TInt)
    fc.SetBody(intExpr 1)

    let fb = FBBuilder("ValidFB")
    fb.AddInput("enable", DsDataType.TBool)
    fb.AddOutput("output", DsDataType.TBool)
