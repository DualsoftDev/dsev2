...
type ScopeType =
    ...
    | FBTemp of instanceName:string * fbName:string      // FB Temp 변수 스코프
    | Temporary        // 임시 스코프
...
type ScopeManager() =
    ...
    member _.ScopeFBStatic(instanceName: string, varName: string) : string =
        sprintf "FB_%s_Static_%s" (sanitizeName instanceName) (sanitizeName varName)

    /// FB Temp 변수에 스코프 적용
    /// 예: "Motor1", "tempResult" → "FB_Motor1_Temp_tempResult"
    member _.ScopeFBTemp(instanceName: string, varName: string) : string =
        sprintf "FB_%s_Temp_%s" (sanitizeName instanceName) (sanitizeName varName)
        ...
        let temps =
            ...
            |> List.map (fun (name, dt) ->
                { OriginalName = name
                  ...
                  Scope = ScopeType.FBTemp(instName, fb.Name)
                  DataType = dt })
...
module NamespaceManager =
    ...
    /// 유효한 식별자인지 검증 (알파벳, 숫자, 언더스코어만 허용, 숫자로 시작 불가)
    let private isValidIdentifier (name: string) : bool =
        if String.IsNullOrWhiteSpace(name) then
            false
        else
            let firstChar = name.[0]
            let isValidFirstChar = Char.IsLetter(firstChar) || firstChar = '_'
            let isValidRest = name |> Seq.skip 1 |> Seq.forall (fun c -> Char.IsLetterOrDigit(c) || c = '_')
            isValidFirstChar && isValidRest
    ...
    let validateNamespace (namespace': string) : Result<unit, string> =
        if String.IsNullOrWhiteSpace(namespace') then
            ...
            let invalidParts =
                parts
                |> Array.filter (fun part -> not (isValidIdentifier part))
...
type ScopeContext = {
    ...
    member this.ResolveName(varName: string) : string option =
        ...
        |> Map.tryPick (fun fcName fc ->
            let fcParams = fc.Inputs @ fc.Outputs
            if fcParams |> List.exists (fun p -> p.Name = varName) then
