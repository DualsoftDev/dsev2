...
#### 0. 링크
  ...
  - [📁 DB schema 보기](./Data/250607/sqlite-schema.sql)
    - [📁 Sqlite DB 보기](./Data/250607/dssystem.sqlite3)
  - [📁 객체 생성 샘플 소스 보기](../../src/unit-test/UnitTest.Core/database/CreateSample.fs)
  - [📁 JSON 저장 샘플 보기](./Data/250607/dssystem.json)
##### 0.1 Cylinder 포함 샘플  
  - [📁 객체 생성 샘플 소스 보기](../../src/unit-test/UnitTest.Core/database/CreateSample.WithCylinders.fs)
  - [📁 JSON 저장 샘플 보기](./Data/250607/dssystem-with-cylinder.json)
  - [📁 Sqlite DB 보기](./Data/250607/dssystem-with-cylinder.sqlite3)
...
### 1.4 저장소
   - EV2 저장소는 크게 파일 저장소와 데이터베이스 저장소로 구분

#### 파일 저장소
   - 기본적으로 하나의 프로젝트 단위가 저장된다.
   - 동적 변경 정보 (e.g R/G/F/H) 는 저장되지 않는다.
   - 정적 정보만 저장되기 때문에, 직접 제어하지 않는 type 이 동일한 복수개의 device 시스템 정보는 한번만 저장되고, 이를 복수개 사용한다는 정보만 기록된다.
   - 하나의 project 에 대해서만, 그 project 를 구성하는 모든 시스템 정보가 저장된다.
   - 현재 JSON format 지원
   - project 하부의 시스템에 대해서는 import/export 지원
   - DB 에 저장되거나 DB 로부터 읽어온 project 는 해당 DB 에 대한 연결 정보와, db 에 저장된 PK 정보도 함께 저장된다.

#### 데이터베이스 저장소
  - 복수개의 project 에 대한 정보를 모두 저장할 수 있다.
  - 동적 정보도 함께 저장/update 된다.
...
### 2.5 Call / Work 사전 조건 (Precondition) 사양서

## 📌 개요

본 문서는 DualSoft DS 시스템 및 DTS 인과 모델에 적용되는 `Call`과 `Work` 단위의 사전 조건(Precondition) 사양을 기술한다. 사전 조건은 작업 흐름 내에서 해당 작업이 시작되기 위한 논리적 인과 조건이며, 자동/수동 운전 모드에 따라 조건의 유효성이 달라진다.

---

## 🔖 Precondition 타입 정의

| 타입                | 대상 범위         | 동작 조건 설명                                      | 작동 모드         |
|---------------------|-------------------|-----------------------------------------------------|-------------------|
| `AutoPrecondition`  | `Call`, `Work`    | 자동 운전 상태일 때만 인과 조건으로 유효            | Auto 모드 전용    |
| `CommonPrecondition`| `Call`, `Work`    | 자동, 수동 운전 모두에서 인과 조건으로 유효          | Auto + Manual     |

---

## 🧠 동작 원리

- `Precondition`은 `Call` 또는 `Work`가 시작(Start)되기 위한 **조건 논리식**으로 사용됨.
- DTS 시스템에서는 각 Segment 또는 Action이 `Ready → Going → Finish → Homing` 상태를 가짐.
- Precondition은 보통 이전 Segment의 `.E` (End) 상태나 `.G` (Going) 상태와 논리적으로 연결됨.

---

## 🔁 상태 기반 예시

| 조건식 유형      | 예시 식 (`DS Language`)                  | 설명                                               |
|------------------|-------------------------------------------|----------------------------------------------------|
| Auto 전용       | `(AutoMode = true) & (X1.E) > X2`         | X1 종료 후 Auto일 때만 X2 시작 가능               |
| 공통 전용       | `(X3.E) > X4`                             | X3 종료되면 X4 실행 (모드 무관)                   |
| Going 상태       | `(X1.S) & (!X1.E)`                        | X1이 진행 중일 때                                  |
| Homing 상태      | `(X1.E) & (X1.R)`                         | X1이 종료되고 복원됨                               |
