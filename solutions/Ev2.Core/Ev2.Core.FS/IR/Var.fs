...
type Variable<'T>(name, ?value:'T, ?varType:VarType, ?initValue:'T, ?comment:string) =
    ...
    member val Comment = comment |? null with get, set
    ...
    interface IVariable<'T> with
        ...
        member x.Value with get() = box x.Value and set(v:obj) = assert(v :? 'T); x.Value <- (v :?> 'T)
        member x.TValue = x.Value
