...
module internal Ds2SqliteImpl =
    ...
    let idUpdator (targets:IUnique seq) (id:int)=
        for t in targets do
            ...
            | :? ArrowBetweenCalls as a -> a.OptId <- Some id
            | :? ArrowBetweenWorks as a -> a.OptId <- Some id
    ...
    let project2Sqlite (proj:DsProject) (connStr:string) (removeExistingData:bool option) =
        ...
        try
            match removeExistingData, proj.OptId with
            | Some true, Some id ->
...
module internal Sqlite2DsImpl =
    ...
    let fromSqlite3(identifier:DbObjectIdentifier) (conn:IDbConnection) (tr:IDbTransaction) =
        ...
        let actives, passives = edSystems |> partition (fun s -> projSysMaps |> tryFind(fun m -> m.SystemId = s.OptId.Value) |-> _.IsActive |? false)
        actives  |> iter edProj.AddActiveSystem
        passives |> iter edProj.AddPassiveSystem

        for s in edSystems do
            let edFlows = [
                for orm in conn.Query<ORMFlow>($"SELECT * FROM {Tn.Flow} WHERE systemId = @SystemId", {| SystemId = s.OptId.Value |}, tr) do
            ...
            let edWorks = [
                for orm in conn.Query<ORMWork>($"SELECT * FROM {Tn.Work} WHERE systemId = @SystemId", {| SystemId = s.OptId.Value |}, tr) do
                    EdWork.Create(orm.Name, s, ?id=n2o orm.Id, guid=s2guid orm.Guid, dateTime=orm.DateTime)
                    |> tee(fun w ->
                        if orm.FlowId.HasValue then
                            let flow = edFlows |> find(fun f -> f.OptId.Value = orm.FlowId.Value)
            ...
            for w in edWorks do
                let edCalls = [
                    for orm in conn.Query<ORMCall>($"SELECT * FROM {Tn.Call} WHERE workId = @WorkId", {| WorkId = w.OptId.Value |}, tr) do
                ...
                // work 내의 call 간 연결
                let edArrows = [
                    for orm in conn.Query<ORMArrowCall>($"SELECT * FROM {Tn.ArrowCall} WHERE workId = @WorkId", {| WorkId = w.OptId.Value |}, tr) do
                        let src = edCalls |> find(fun c -> c.OptId.Value = orm.Source)
                        let tgt = edCalls |> find(fun c -> c.OptId.Value = orm.Target)
            ...
            // system 내의 work 간 연결
            let edArrows = [
                for orm in conn.Query<ORMArrowWork>($"SELECT * FROM {Tn.ArrowWork} WHERE systemId = @SystemId", {| SystemId = s.OptId.Value |}, tr) do
                    let src = edWorks |> find(fun w -> w.OptId.Value = orm.Source)
                    let tgt = edWorks |> find(fun w -> w.OptId.Value = orm.Target)
