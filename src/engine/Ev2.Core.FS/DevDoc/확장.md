...
## 🆕 새로운 createExtended<T>() 패턴 (v1.4)

### 개요

DSE v2에 새로운 객체 생성 패턴인 `createExtended<T>()`가 추가되었습니다. 이 패턴은 서브클래스(확장 타입)에서 기본 생성자로 객체를 생성한 후, `Initialize` 메서드로 초기화하는 방식입니다.

### 기본 사용법

```fsharp
// 새로운 패턴: 제네릭 타입으로 직접 생성
let extended = createExtended<DsSystem>()    // DsSystem subclass object 생성
...
새로운 `createExtended<T>()` 패턴은 기존 확장 시스템과 완벽하게 통합됩니다:
...
// 사용 시
var customSystem = createExtended<DsSystem>();  // CustomSystem 인스턴스가 생성됨
customSystem.Initialize(flows, works, arrows, apiDefs, apiCalls);
```

### 기존 패턴과의 호환성

기존 `createExtensible` 패턴도 계속 지원되므로 호환성에 문제없습니다:
...
모든 핵심 타입에 매개변수 없는 기본 생성자가 추가되었습니다:
...
1. **타입 안전성**: 제네릭 타입으로 컴파일 타임에 타입 검사
2. **간결성**: 람다 함수 없이 직접 타입 지정
3. **유연성**: Initialize 메서드로 단계별 초기화 가능
4. **확장성**: 서브클래스에서 자연스럽게 동작
5. **호환성**: 기존 패턴과 완벽 호환

### 마이그레이션 가이드

기존 코드를 새로운 패턴으로 마이그레이션하는 방법:
...
- **v1.4**: 새로운 `createExtended<T>()` 패턴 및 Initialize 메서드 추가

이 확장 시스템을 통해 DSE v2의 핵심 기능을 손상시키지 않으면서도 비즈니스 요구사항에 맞는 커스터마이징이 가능합니다. 새로운 `createExtended<T>()` 패턴으로 더욱 간결하고 타입 안전한 객체 생성이 가능해졌습니다.